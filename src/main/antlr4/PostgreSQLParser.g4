...
func_as locals[org.antlr.v4.runtime.ParserRuleContext Definition]
   :
   /* |AS 'definition'*/
   def = sconst
   /*| AS 'obj_file', 'link_symbol'*/
...
indirection_el
   // PATCH (SemanticScope): (attr_name | STAR) made optional so a trailing dot
   // (e.g. "u.", mid-typing "u.na") stays syntactically valid instead of
   // triggering ANTLR error-recovery. Mirrors the old qualifiedName "DOT??"
   // patch - see SemanticScope.checkDanglingDot().
   : DOT (attr_name | STAR)?
