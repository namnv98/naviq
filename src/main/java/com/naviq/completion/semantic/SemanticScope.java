...
/**
 * Build scope (alias -> table) + phát hiện "đang gõ dở sau dấu chấm", chịu được
 * input lỗi/gõ dở nhờ 3 nguyên tắc mirror DBeaver:
 *
 * 1. Grammar patch "qualifiedName ... DOT??" (xem grammar.patch.md) - biến
 *    "u." thành hợp lệ về cú pháp, KHÔNG sinh lỗi, KHÔNG kích hoạt ANTLR
 *    error-recovery (nguồn gốc bug "cắn nhầm alias" ban đầu).
 * 2. Mọi callback dưới đây defensive - null/thiếu ở đâu thì bỏ qua đúng chỗ đó,
 *    KHÔNG throw, KHÔNG làm hỏng phần scope đã build được (mirror
 *    SQLQuerySemanticUtils.performPartialResolution của DBeaver).
 * 3. hasErrorChildren() (mirror STMTreeNode.hasErrorChildren()) - phát hiện
 *    subtree bị ANTLR error-recovery vá vào (xem PostgreSQLParserBase +
 *    STMTreeTermErrorNode) để KHÔNG đăng ký alias/tên dựa trên dữ liệu
 *    không đáng tin, thay vì mù quáng tin mọi thứ ANTLR trả về.
 */
public class SemanticScope extends com.example.PostgreSQLBaseListener {
    ...
    public static class Scope {
        ...
        /**
         * Tên cột mà CHÍNH scope này PROJECT ra (SELECT list của nó) - dùng khi scope
         * này được dùng làm nguồn cho 1 alias khác (subquery/CTE) để biết nó "trả ra
         * cột gì". Chỉ chứa cột suy ra được tên (có AS alias, hoặc là 1 columnName đơn
         * giản không alias) - biểu thức phức tạp không alias bị bỏ qua (không suy ra
         * được tên cột thật, giống Postgres tự đặt tên kiểu "?column?").
         */
        public final List<String> projectedColumns = new ArrayList<>();

        /**
         * true nếu SELECT list có ít nhất 1 "*" (bare "SELECT *" hoặc "alias.*") -
         * nghĩa là projectedColumns KHÔNG đủ để biết hết cột output, còn phụ thuộc
         * cột thật của (các) bảng nguồn trong FROM - caller cần tự mở rộng qua schema.
         */
        public boolean hasWildcard = false;

        /**
         * Giống aliases, nhưng CHỈ chứa các alias trỏ tới subquery/CTE (không phải
         * bảng thật) - lưu THẲNG tham chiếu Scope, không cần parse ngược chuỗi
         * "<cte#N>"/"<subquery#N>" như trước. Với alias trỏ tới bảng thật, entry
         * này KHÔNG tồn tại (chỉ có trong aliases).
         */
        public final Map<String, Scope> derivedScopeAliases = new LinkedHashMap<>();

        Scope(int id, Scope parent) { this.id = id; this.parent = parent; }
        ...
        public Map<String, String> visibleAliases() {
            ...
            for (Scope s = this; s != null; s = s.parent) chain.push(s);
            Map<String, String> result = new LinkedHashMap<>();
            while (!chain.isEmpty()) result.putAll(chain.pop().aliases);
            return result;
        }
        ...
        public Map<String, Scope> visibleDerivedScopes() {
            ...
            while (!chain.isEmpty()) result.putAll(chain.pop().derivedScopeAliases);
            return result;
        }
    }
    ...
    /**
     * Token index bị báo lỗi bởi ErrorListener (offendingSymbol) - PHẢI truyền vào
     * từ ngoài (xem SemanticScopeTest) vì đây là nguồn đáng tin cậy DUY NHẤT cho
     * MỌI kiểu lỗi, kể cả kiểu ANTLR không tạo ErrorNode trong cây (single-token
     * deletion cho "extraneous input" - xem ghi chú ở isUnreliable()).
     */
    public final Set<Integer> offendingTokenIndices = new HashSet<>();

    /**
     * Mirror STMTreeNode.hasErrorChildren() của DBeaver, NHƯNG bổ sung thêm 1 nguồn
     * phát hiện lỗi thứ 2 vì thực nghiệm cho thấy hasErrorChildren-qua-ErrorNode
     * KHÔNG đáng tin cho mọi trường hợp:
     *
     * - Lỗi "thiếu token" (vd "u." thiếu identifier sau dấu chấm) -> ANTLR chèn
     *   token ảo (tokenIndex == -1) -> LUÔN tạo ErrorNode -> instanceof check đủ.
     * - Lỗi "token thừa" (vd "123abc" - số không hợp lệ ở vị trí identifier) ->
     *   ANTLR xóa token thừa bằng single-token-deletion -> KHÔNG đảm bảo tạo
     *   ErrorNode (đã xác nhận bằng thực nghiệm) -> phải cross-check bằng
     *   offendingTokenIndices (luôn có từ ErrorListener bất kể kiểu lỗi nào).
     */
    public boolean isUnreliable(org.antlr.v4.runtime.ParserRuleContext ctx) {
        ...
        if (ctx.getStart() != null && ctx.getStop() != null) {
            ...
            for (int idx : offendingTokenIndices) {
                if (idx >= a && idx <= b) { intervalFlag = true; break; }
            }
        }
        if (debug) {
    ...
    /**
     * Mirror STMTreeNode.hasErrorChildren() của DBeaver: duyệt đệ quy toàn bộ
     * subtree, true nếu có BẤT KỲ node nào là STMTreeTermErrorNode (tức ANTLR
     * đã phải error-recovery để vá vào chỗ này - không phải input thật).
     */
    public static boolean hasErrorChildren(ParseTree node) {
        if (node instanceof ErrorNode) return true;
        for (int i = 0; i < node.getChildCount(); i++) {
            if (hasErrorChildren(node.getChild(i))) return true;
        }
        return false;
    }

    /**
     * Scope chứa tokenIndex - PHẢI dùng interval containment [start, stop], không
     * chỉ so startTokenIndex (bug đã xác nhận: scope con đã ĐÓNG từ lâu, nếu chỉ so
     * startTokenIndex lớn nhất <= tokenIndex, sẽ bị chọn nhầm thay vì scope cha đang
     * bao trọn tokenIndex thật sự - vd cursor ở WHERE của outer query, sau khi 1
     * subquery lồng sâu trong FROM đã đóng). Trong các scope có interval chứa
     * tokenIndex, ưu tiên interval HẸP NHẤT (tức sâu/cụ thể nhất).
     */
    public Scope scopeAt(int tokenIndex) {
        Scope best = null;
        for (Scope s : allScopes) {
            if (s.startTokenIndex < 0) continue; // root hoặc chưa set
    ...
    @Override
    public void enterSelectStmt(com.example.PostgreSQLParser.SelectStmtContext ctx) {
        pushScope(ctx.getStart());
    }

    @Override
    public void exitSelectStmt(com.example.PostgreSQLParser.SelectStmtContext ctx) {
    ...
    @Override
    public void enterUpdateStmt(com.example.PostgreSQLParser.UpdateStmtContext ctx) {
        Scope child = pushScope(ctx.getStart());
        registerDmlTableAlias(child, ctx.tableName(), ctx.tableAlias());
    }

    @Override
    public void exitUpdateStmt(com.example.PostgreSQLParser.UpdateStmtContext ctx) {
        popScope(ctx.getStop());
    }

    @Override
    public void enterDeleteStmt(com.example.PostgreSQLParser.DeleteStmtContext ctx) {
        Scope child = pushScope(ctx.getStart());
        registerDmlTableAlias(child, ctx.tableName(), ctx.tableAlias());
    }

    @Override
    public void exitDeleteStmt(com.example.PostgreSQLParser.DeleteStmtContext ctx) {
        popScope(ctx.getStop());
    }
    ...
    private void registerDmlTableAlias(
        Scope target,
        com.example.PostgreSQLParser.TableNameContext tableNameCtx,
        com.example.PostgreSQLParser.TableAliasContext aliasCtx
    ) {
        if (target == null || tableNameCtx == null) return;
        ...
        if (isUnreliable(tableNameCtx)) return;
        if (aliasCtx != null && isUnreliable(aliasCtx)) return;
    ...
    @Override
    public void exitSelectElements(com.example.PostgreSQLParser.SelectElementsContext ctx) {
    ...
    @Override
    public void exitSelectElement(com.example.PostgreSQLParser.SelectElementContext ctx) {
        Scope cur = stack.peek();
        if (cur == null || isUnreliable(ctx)) return;
        ...
        if (outName != null) cur.projectedColumns.add(outName);
    }

    // ---- FROM/JOIN alias ----

    @Override
    public void enterSimpleTable(com.example.PostgreSQLParser.SimpleTableContext ctx) {
        Scope cur = stack.peek();
        if (cur == null || ctx.tableName() == null) return; // gõ dở -> bỏ qua, không throw
    ...
    /**
     * Tìm "table" trong danh sách CTE đã ĐÓNG (exitCteDefinition đã chạy) tính tới
     * thời điểm này - gồm CTE trước trong CÙNG WITH (pendingCte, chưa merge vào scope)
     * và CTE của WITH bao ngoài đã merge xong vào scope cha. Trả về CHÍNH Scope của
     * CTE đó nếu khớp, null nếu "table" thực sự là 1 bảng độc lập.
     */
    ...
    @Override
    public void exitSubqueryTable(com.example.PostgreSQLParser.SubqueryTableContext ctx) {
        ...
        if (ctx.tableAlias() == null) return;
        if (isUnreliable(ctx)) return; // check toàn bộ ctx, không chỉ tableAlias() riêng lẻ
        Scope outer = stack.peek();
        if (outer == null || outer.children.isEmpty()) return;
    ...
    @Override
    public void enterWithClause(com.example.PostgreSQLParser.WithClauseContext ctx) {
        withHost.push(stack.peek());
        pendingCte.push(new LinkedHashMap<>());
    }

    @Override
    public void exitCteDefinition(com.example.PostgreSQLParser.CteDefinitionContext ctx) {
        if (pendingCte.isEmpty() || ctx.cteName() == null) return;
        if (isUnreliable(ctx)) return; // check toàn bộ ctx, không chỉ cteName() riêng lẻ
        Scope host = withHost.peek();
        if (host == null || host.children.isEmpty()) return;
        pendingCte.peek().put(ctx.cteName().getText(), host.children.get(host.children.size() - 1));
    }

    @Override
    public void exitWithClause(com.example.PostgreSQLParser.WithClauseContext ctx) {
    ...
    @Override
    public void exitQualifiedName(com.example.PostgreSQLParser.QualifiedNameContext ctx) {
        ...
        for (var child : ctx.children) {
            boolean isDot = child instanceof TerminalNode t
                && t.getSymbol().getType() == com.example.PostgreSQLLexer.DOT;
            if (isDot) {
     ...
     *
     * visibleAliases: LUÔN có giá trị hữu ích ở MỌI vị trí (kể cả cursor đứng ở chỗ
     *                 hoàn toàn trống) - alias -> tên bảng thật HOẶC "<cte#N>"/
     *                 "<subquery#N>" (dạng hiển thị, dùng để debug/log).
     * visibleDerivedScopes: alias -> Scope thật, CHỈ chứa các alias trỏ tới subquery/
     *                 CTE (không có entry cho alias trỏ tới bảng thật). Dùng khi
     *                 KHÔNG có danglingQualifier (cursor không đứng sau dấu chấm) -
     *                 với MỖI alias trong visibleAliases, nếu nó CÓ mặt ở đây thì lấy
     *                 cột qua .projectedColumns/.hasWildcard; nếu KHÔNG có thì đó là
     *                 bảng thật, tự tra schema (SemanticScope không biết schema).
     * danglingQualifier: chỉ khác null nếu cursor đứng NGAY SAU 1 dấu chấm cụt (vd "u.")
     *                 - là phần đứng trước dấu chấm đó (vd "u").
     * danglingQualifierResolvesTo: dạng hiển thị (tên bảng thật hoặc "<cte#N>"/
     *                 "<subquery#N>") của alias đó - null nếu alias lạ/gõ sai.
     * danglingQualifierScope: Scope THẬT nếu qualifier trỏ tới subquery/CTE (dùng
     *                 .projectedColumns/.hasWildcard trực tiếp) - null nếu qualifier
     *                 trỏ tới bảng thật (tự tra schema) hoặc alias lạ.
     */
    ...
    /**
     * Chèn 1 identifier giả (CURSOR_PLACEHOLDER) NGAY TẠI cursorOffset, TRỪ KHI ký tự
     * ngay trước cursor là dấu chấm - trường hợp đó để nguyên, vì DOT?? trong grammar
     * đã xử lý đúng rồi (chèn thêm vào sẽ PHÁ cơ chế phát hiện "chấm cụt").
     *
     * Lý do cần: những vị trí HOÀN TOÀN TRỐNG (vd "SELECT |FROM t", "WHERE |") không
     * có gì để grammar "bám" vào - ANTLR error-recovery (single-token deletion) có
     * thể đoán sai, xóa nhầm 1 từ khóa CẤU TRÚC thật (vd FROM) thay vì hiểu đây là
     * "người dùng đang gõ dở". Chèn 1 identifier giả biến vị trí đó thành hợp lệ về
     * cú pháp, loại bỏ hẳn tình huống gây đoán sai.
     */
    public static String withCursorPlaceholder(String sql, int cursorOffset) {
        ...
        if (rightAfterDot) return sql; // DOT?? tự lo, không đụng vào
