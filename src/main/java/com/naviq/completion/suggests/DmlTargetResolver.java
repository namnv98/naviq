...
//
///**
// * Xác định bảng đích của INSERT/UPDATE/ALTER khi vị trí cursor KHÔNG đứng sau dấu chấm (nên không
// * có "qualifier" theo nghĩa alias.column) - đây là bài toán khác hẳn với SemanticScope: "câu này
// * thuộc loại statement nào, và bảng đích của nó là gì" thay vì "alias này trỏ tới bảng nào". Cũng
// * dùng làm fallback token-scan khi SemanticScope parse lỗi nặng (xem SemanticAnalyzer).
// */
...
//                || type == PostgreSQLParser.DELETE) {
//                return null;
//            }
//
//            if (type == PostgreSQLParser.COLUMN) {
//                int k = i - 1;
//                while (k >= 0 && tokens.get(k).getChannel() != Token.DEFAULT_CHANNEL) {
//                    k--;
//                }
//                if (k < 0) {
//                    return null;
...
//                    && tokens.get(k).getChannel() != Token.DEFAULT_CHANNEL) {
//                    k++;
//                }
...
//                continue;
//            }
//            if (tokens.get(i).getType() != PostgreSQLParser.TABLE) {
//                continue;
//            }
//
//            int k = i - 1;
...
//            int j = i + 1;
//            while (j < fromIndex
//                && tokens.get(j).getChannel() != Token.DEFAULT_CHANNEL) {
//                j++;
//            }
//            if (j >= fromIndex || tokens.get(j).getType() != PostgreSQLParser.ID) {
//                return null;
//            }
//
...
//            int j2 = j + 1;
//            while (j2 < fromIndex
//                && tokens.get(j2).getChannel() != Token.DEFAULT_CHANNEL) {
//                j2++;
//            }
//
...
//                int j3 = j2 + 1;
//                while (j3 < fromIndex
//                    && tokens.get(j3).getChannel() != Token.DEFAULT_CHANNEL) {
//                    j3++;
//                }
...
//                }
//                if (k >= caretTokenIndex
//                    || tokens.get(k).getType() != PostgreSQLParser.ID) {
//                    return null;
//                }
//
...
//                    && tokens.get(k2).getChannel() != Token.DEFAULT_CHANNEL) {
//                    k2++;
//                }
//
...
//                        && tokens.get(k3).getChannel() != Token.DEFAULT_CHANNEL) {
//                        k3++;
//                    }
...
//                }
//
//                if (tokens.get(k).getType() != PostgreSQLParser.ID) {
//                    return null;
//                }
