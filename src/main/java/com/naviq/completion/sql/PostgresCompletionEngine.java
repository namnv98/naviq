package com.naviq.completion.sql;

import com.example.PostgreSQLLexer;
import com.example.PostgreSQLParser;
import com.naviq.completion.draft.CollectTableInfo;
...
import com.naviq.completion.sql.context.SqlContextCollector;
...
public class PostgresCompletionEngine {
    ...
    static int findCaretTokenIndex(CommonTokenStream tokenStream, int cursorCharPos) {
        ...
        for (int i = 0; i < tokens.size() - 1; i++) {
            ...
            // token bao phủ cursor → đây là token cần complete
            if (t.getStartIndex() <= cursorCharPos && cursorCharPos <= t.getStopIndex()) return i;
            // cursor nằm trong whitespace trước token này → token này là vị trí cần complete
    ...
    public static void main(String[] args) {
        var suggests = suggests("select * from ( select ( select ( select ( select ( select d4.id as k from demo as d4 where d4.id = 0) from demo as d3 where d3.id = 0) from demo as d2 where d2.id = 0) from demo as d1 where d1.id = 0) from demo as d) where k >1",
                "select * from ( select ( select ( select ( select ( select d4.".length());
        System.out.println();
        System.out.println();
    }
    ...
    public static List<Suggest> suggests(String sql, Integer cursorCharPos) {
        ...
        var sqlContextBuilder1 = new com.naviq.completion.draft.SqlContextCollector(TABLE_INDEX);
        com.naviq.completion.draft.SqlContextCollector.SqlContext sqlContext1 = sqlContextBuilder1.collect(tokenStream, caretTokenIndex);

        var sqlContextCollector = new SqlContextCollector(TABLE_INDEX);
        SqlContext sqlContext = sqlContextCollector.collect(tokenStream, caretTokenIndex);
        ...
        AntlrCompletionEngine.CandidatesCollection candidates = completionEngine.collectCandidates(caretTokenIndex);
        ...
        for (Map.Entry<Integer, List<AntlrCompletionEngine.RuleContext>> entry : candidates.rules.entrySet()) {
            ...
            if (ruleName.equals("columnName")) {
                if (qualifier != null) {
                    String realTable = sqlContext.aliasMap().getOrDefault(qualifier, qualifier);
                    if (sqlContext.cteColumns().containsKey(realTable)) {
                        sqlContext.cteColumns().get(realTable).forEach(c -> {
                            suggests.add(Suggest.of(qualifier + "." + c.name(), "column"));
                        });
                    } else {
                    ...
                    if (sqlContext.columns().isEmpty()) {
                        SCHEMA_TABLE_INDEX.keySet().forEach(t -> getColumnsOfTable(t).forEach(c -> {
                            suggests.add(Suggest.of(c.fullName(), "column", c.dataType()));
                        }));
                    } else {
                        sqlContext.columns().forEach(columnInfo -> {
                            if (isNull(columnInfo.tableAlias())) {
                                suggests.add(Suggest.of(columnInfo.nameAlias(), "column", columnInfo.type()));
                            } else {
                                suggests.add(Suggest.of(columnInfo.tableAlias() + "." + columnInfo.name(), "column", columnInfo.type()));
                            }
                        });
                    }
                }
//                FUNCTIONS.forEach(fn -> suggests.add(Suggest.of(fn, "function")));
            ...
            if (ruleName.equals("tableAlias")) {
                ...
                if (tableName != null) {
                    String alias = suggestAlias(sqlContext, tableName);
    ...
    static String suggestAlias(SqlContext visitor, String tableName) {
        if (tableName == null || tableName.isEmpty()) return null;

        var keySet = visitor.aliasMap().keySet();
