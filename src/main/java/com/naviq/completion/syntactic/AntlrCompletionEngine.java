...
public class AntlrCompletionEngine {
    ...
    private Set<Integer> traverseATN(ATNState start, int tokenIndex, RuleCallStack stack) {
        ...
        if (tokenIndex >= tokens.size() - 1) {   // at caret
            ...
            return followSets.combined.contains(org.antlr.v4.runtime.Token.EPSILON) ? Collections.singleton(tokenIndex) : Collections.emptySet();
        }

        InputToken cur = tokens.get(tokenIndex);
        boolean canProceed = followSets.combined.contains(org.antlr.v4.runtime.Token.EPSILON) || followSets.combined.contains(cur.type);
        if (!canProceed) {
            return Collections.emptySet();
        }

        return runBFS(start, tokenIndex, entered);
    }
    ...
    private static void collectFollowSets(Parser parser, ATNState s, ATNState stop,
        ...
        for (Transition t : s.getTransitions()) {
            if (t instanceof RuleTransition rt) {
                ...
                out.add(new FollowSetWithPath(IntervalSet.of(org.antlr.v4.runtime.Token.MIN_USER_TOKEN_TYPE, parser.getATN().maxTokenType), ruleStack.copy(), Collections.emptyList()));
            } else if (t.isEpsilon()) {
                collectFollowSets(parser, t.target, stop, out, seen, ruleStack, ignoredTokens, returnStates);
            } else {
                IntervalSet label = t.label();
                ...
                if (t instanceof NotSetTransition) {
                    label = label.complement(org.antlr.v4.runtime.Token.MIN_USER_TOKEN_TYPE, parser.getATN().maxTokenType);
                }
