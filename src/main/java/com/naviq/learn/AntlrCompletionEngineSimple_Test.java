 ...
 */
public class AntlrCompletionEngineSimple_Test {
    ...
    // [SỬA] Map thay vì Set để giữ following list như C3
    private final Map<Integer, List<Integer>> suggestedTokens = new HashMap<>();
    ...
    // [THÊM] Cache follow sets dùng chung - GIỐNG followSetsByATN của C3
    private final Map<Integer, FollowSetsHolder> followSetsCache = new HashMap<>();

    // [THÊM] Path hiện tại - GIỐNG callStack của C3, để quét outer-most preferred rule
    private final List<Integer> currentPath = new ArrayList<>();

    public AntlrCompletionEngineSimple_Test(Parser parser, Map<Integer, Boolean> ignoredTokens, Map<Integer, Boolean> preferredRules) {
        this.parser = parser;
    ...
    private Set<Integer> enterRule(ATNState start, int tokenIndex) {
        ...
        // [THÊM] Thêm rule vào path - GIỐNG C3 callStack
        currentPath.add(start.ruleIndex);
        ...
        // [THÊM] Xóa rule khỏi path sau khi xử lý
    ...
    private Set<Integer> handleReachedCaretInsideRule(ATNState start, int tokenIndex) {
        // [SỬA] KHÔNG dừng ở preferred-rule - giống C3, đi xuyên qua để tìm tokens
        // if (preferredRules.containsKey(start.ruleIndex)) {
        //     suggestedRules.add(start.ruleIndex);
        //     ...
        // }

        // [THÊM] GIỐNG C3: translate preferred rule và thêm tokens từ follow sets
        FollowSetsHolder followSets = getFollowSets(start);
        for (FollowSetWithPath set : followSets.sets) {
            List<Integer> fullPath = new ArrayList<>(currentPath);
            fullPath.addAll(set.path);

            if (!translateToRuleIndex(fullPath)) {
                List<Integer> list = set.intervals.toList();
                boolean addFollowing = list.size() == 1;
                for (int sym : list) {
                    if (!ignoredTokens.containsKey(sym)) {
                        if (addFollowing && set.following != null && !set.following.isEmpty()) {
                            suggestedTokens.put(sym, new ArrayList<>(set.following));
                        } else {
                            suggestedTokens.put(sym, new ArrayList<>());
                        }
                    }
                }
            }
        ...
        // Kiểm tra nullable để biết rule này có thể kết thúc không cần token không
    ...
    private Set<Integer> walkRuleBody(ATNState start, int startTokenIndex) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (cur.state.getStateType() == ATNState.RULE_STOP) {
                // [SỬA] Gọi translate khi gặp RULE_STOP ở caret - GIỐNG C3
                if (isAtCaret(cur.tokenIndex)) {
                    translateToRuleIndex(currentPath);
                }
            ...
            for (Transition t : cur.state.getTransitions()) {
                if (t instanceof RuleTransition rt) {
                    ...
                    // [THÊM] Gọi translate khi gặp epsilon ở caret - GIỐNG C3
                    if (atCaret) {
                        translateToRuleIndex(currentPath);
                    }
     ...
     * <p>
     * [SỬA] Bỏ runningRuleIndex - việc chọn preferred rule được xử lý bởi translateToRuleIndex
     */
    private void handlePasswordDoor(Transition t, PipelineEntry cur, boolean atCaret, Deque<PipelineEntry> queue) {
        ...
        if (atCaret) {
            // [SỬA] KHÔNG dừng ở preferred-rule - giống C3, chỉ thêm tokens
    ...
    private void handleWildcardDoor(WildcardTransition t, PipelineEntry cur, boolean atCaret, Deque<PipelineEntry> queue) {
        ...
        // [SỬA] KHÔNG dừng ở preferred-rule - giống C3
        // if (preferredRules.containsKey(runningRuleIndex)) {
        //     suggestedRules.add(runningRuleIndex);
        //     return;
        // }
    ...
    // ════════════════════════════════════════════════════════════════
    // [THÊM] FOLLOW SETS - GIỐNG C3
    // ════════════════════════════════════════════════════════════════

    /**
     * Lấy hoặc tính follow sets cho rule - GIỐNG C3
     */
    private FollowSetsHolder getFollowSets(ATNState start) {
        return followSetsCache.computeIfAbsent(start.stateNumber, k -> {
            FollowSetsHolder holder = new FollowSetsHolder();
            RuleStopState stop = atn.ruleToStopState[start.ruleIndex];

            List<FollowSetWithPath> sets = determineFollowSets(start, stop);
            holder.sets = sets;
    ...
    /**
     * GIỐNG C3: collectFollowSets - đi xuyên qua preferred rules
     */
    private void collectFollowSets(ATNState s, ATNState stopState,
                                   List<FollowSetWithPath> followSets,
                                   Set<ATNState> seen, List<Integer> ruleStack) {
        ...
        for (Transition transition : s.getTransitions()) {
            switch (transition.getSerializationType()) {
                case Transition.RULE: {
                    ...
                    ruleStack.add(rt.target.ruleIndex);
    ...
    // ════════════════════════════════════════════════════════════════
    // [THÊM] TRANSLATE TO PREFERRED RULE - GIỐNG C3
    // ════════════════════════════════════════════════════════════════

    /**
     * GIỐNG C3: translateToRuleIndex()
     * Quét path từ ngoài vào trong, chọn preferred-rule đầu tiên
     */
    private boolean translateToRuleIndex(List<Integer> rulePath) {
        if (preferredRules.isEmpty()) return false;
        if (rulePath.isEmpty()) return false;

        for (int i = 0; i < rulePath.size(); i++) {
            int ruleIdx = rulePath.get(i);
            if (preferredRules.containsKey(ruleIdx)) {
                if (!suggestedRules.contains(ruleIdx)) {
                    suggestedRules.add(ruleIdx);
                }
                return true;
            }
        }
    ...
    private boolean checkPredicate(PredicateTransition pt) {
        return pt.getPredicate().eval(parser, ParserRuleContext.EMPTY);
    }

    // ════════════════════════════════════════════════════════════════
    // CLASSES DỮ LIỆU - GIỐNG C3
    // ════════════════════════════════════════════════════════════════
