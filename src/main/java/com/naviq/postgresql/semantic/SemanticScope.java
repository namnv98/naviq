...
import com.naviq.antlr4.postgresql.PostgreSQLParserBaseListener;
...
public class SemanticScope extends PostgreSQLParserBaseListener {
    ...
    public static class Scope {
        ...
        /**
         * Danh sách scope từ NGOÀI VÀO TRONG (root trước, chính scope này cuối cùng), bỏ qua
         * scope tổ tiên nào là isDdlTargetScope (xem javadoc field đó) - dùng CHUNG cho cả
         * visibleAliases() lẫn visibleDerivedScopes() thay vì mỗi hàm tự leo cây riêng, để
         * SemanticScope.resolveAt() (cần CẢ HAI cùng lúc) chỉ phải leo cây parent MỘT LẦN thay
         * vì hai lần.
         */
        private List<Scope> visibilityChain() {
    ...
    public boolean isUnreliable(org.antlr.v4.runtime.ParserRuleContext ctx) {
        // TỐI ƯU: nếu offendingTokenIndices RỖNG HOÀN TOÀN (kể cả token ảo index -1 từ lỗi
        // "thiếu token"), thì KHÔNG THỂ có ErrorNode nào trong TOÀN BỘ cây - vì ParseTreeWalker
        // duyệt post-order, nên tới lúc 1 exitXxx() nào đó chạy, mọi ErrorNode trong subtree của
        // chính ctx đó đã được visitErrorNode() ghi nhận rồi (bất kể token thật hay ảo). Do đó
        // hasErrorChildren(ctx) CHẮC CHẮN trả false ở đây -> bỏ qua thẳng, không cần duyệt đệ quy
        // toàn bộ subtree (vốn có thể lớn, vd table_ref của 1 chuỗi JOIN dài, hay target_el của 1
        // SELECT list nhiều cột). Đây là trường hợp PHỔ BIẾN NHẤT (input hợp lệ, không lỗi) nên
        // short-circuit này giúp ích ở gần như mọi lần gọi thực tế.
        // LƯU Ý: không được rút gọn thành "return false" luôn nếu offendingTokenIndices rỗng, vì
        // vẫn cần chạy nhánh debug logging bên dưới khi debug=true.
    ...
    public CompletionResult resolveAt(int cursorOffset, Scope scopeAtCursor) {
        // TỐI ƯU: trước đây gọi visibleAliases() + visibleDerivedScopes() riêng, mỗi hàm tự leo
        // cây parent (kèm check isDdlTargetScope) một lần -> leo cây 2 LẦN cho CÙNG 1 chain. Giờ
        // leo cây đúng 1 LẦN qua visibilityChain() rồi build cả 2 map trong cùng 1 vòng lặp.
