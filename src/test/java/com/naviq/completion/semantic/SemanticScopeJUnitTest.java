...
/**
 * Bộ test JUnit 5 cho SemanticScope - thay cho bản main()-based cũ (in PASS/FAIL
 * thủ công). Chạy được qua "mvn test", tích hợp IDE/CI, mỗi case là 1 @Test riêng
 * nên fail case nào biết ngay case đó, không cần đọc log từ đầu tới cuối.
 *
 * Toàn bộ giá trị kỳ vọng giữ NGUYÊN từ bộ test main()-based đã chạy thực nghiệm và
 * xác nhận đúng qua nhiều vòng review trước đó - đây chỉ là đổi HÌNH THỨC chạy/assert,
 * KHÔNG đổi ý nghĩa test.
 */
class SemanticScopeJUnitTest {
    ...
    private static Resolved resolveSingle(String originalSql, int cursorOffset) {
        ...
        parser.addErrorListener(new BaseErrorListener() {
            @Override
            public void syntaxError(Recognizer<?, ?> r, Object offendingSymbol, int l, int c, String m, RecognitionException e) {
    ...
    /** Trả về (projectedColumns, hasWildcard) của derived scope tại vị trí cursor - dùng riêng cho nhóm test projection. */
    ...
    class BasicCases {
        ...
        void twoTableJoin() {
            var r = resolveOne("select * from orders o join customers c on o.customer_id = c.id where c.| ");
        ...
        void tripleNestedCorrelatedSubquery() {
            var r = resolveOne("select (select (select * from x where u.| ) from y) from users as u");
        ...
        void cteReferencingEarlierCte() {
            var r = resolveOne("with a as (select * from t1), b as (select * from a where a.| ) select * from b");
        ...
        void threeTableJoinChain() {
            var r = resolveOne("select * from a ja join b jb on ja.id = jb.a_id join c jc on jb.id = jc.b_id where jc.| ");
        ...
        void twoDanglingDotsInSameStatement() {
            var results = resolveAll("select u.| , o.| from users u join orders o on u.id = o.user_id");
        ...
        void selfJoin() {
            var r = resolveOne("select * from employees e1 join employees e2 on e1.manager_id = e2.id where e2.| ");
    ...
    class ExtendedCasesGroup1 {
        ...
        void existsSubquery() {
            var r = resolveOne("select * from orders o where exists (select 1 from customers c where c.id = o.| )");
            assertEquals(aliases("o", "orders", "c", "customers"), r.aliases());
        ...
        void complexWhereClause() {
            var r = resolveOne("select * from t1 a join t2 b on a.id = b.id where (a.x = 1 and b.y = 2) and a.| ");
    ...
    class PlaceholderCases {
        ...
        void scopeAtPicksInnermostContainingIntervalNotStaleChild() {
            var r = resolveOne("select * from (select (select id from contracts as c2) as id from contracts as c1) as c where |");
    ...
    class ExtendedCasesGroup2 {
        ...
        void threeLevelCteChain() {
            var r = resolveOne("with a as (select * from t1), b as (select * from a), c as (select * from b where b.| ) select * from c");
    ...
    class ProjectionColumns {
        ...
        void cteWithExplicitColumnNames() {
            var p = resolveProjection("with c as (select id, name as full_name from t) select c.| from c");
